...
launch() {
  ...
  nohup jupyter notebook \
    ...
    --log-level=CRITICAL \
    > $LOG/jupyter_app.log 2> $LOG/jupyter_err.log &
  if [[ $HOSTNAME == *"inetpsa.com" ]]; then
    url="http://$HOSTNAME:$port"
  elif [[ $HOSTNAME == *"inetpsa" ]]; then
    url="http://$HOSTNAME.com:$port"
...
remove_logs() {
  if [ -f $LOG/jupyter_app.log ]
	then
	   rm $LOG/jupyter_app.log
	fi

	if [ -f $LOG/jupyter_err.log ]
	then
	   rm $LOG/jupyter_err.log
	fi
}
